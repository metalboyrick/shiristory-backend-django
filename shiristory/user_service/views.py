...
def signup_view(request):
    ...
    bio = "Default Bio"
    profile_pic_url = (
        f"{settings.APP_URL}:{settings.APP_PORT}{settings.MEDIA_URL}sample.jpg"
    )
...
def profile_view(request):
    ...
    # PUT request
        ...
        logged_in_user.nickname = request.data.get(
...
def add_friend(request, friend_username):
    try:
        ...
        if friend_username == logged_in_user.username:
            return JsonResponse({"message": "Cannot add self as friend"})
